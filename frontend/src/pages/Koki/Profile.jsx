...
function KokiProfile() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3000/karyawan/${karyawanId}`)
      .then((res) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:3000/karyawan/3`, {
        nama: editData.nama,
    ...
    <div className='min-h-screen bg-gradient-to-r from-indigo-50 via-blue-50 to-indigo-100 p-8'>
      ...
      <div className='bg-white rounded-lg shadow-xl p-6 max-w-4xl mx-auto space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 text-lg text-gray-700'>
          <p>
            <span className='font-medium text-gray-600'>ID:</span> {karyawan.id}
          </p>
          <p>
          ...
          <p>
            ...
            <span
              className={`font-semibold ${
                karyawan.aktif ? 'text-green-600' : 'text-red-600'
              }`}
            >
          ...
          <p>
            <span className='font-medium text-gray-600'>Clocked In:</span>{' '}
            {karyawan.clockedInAt
              ? new Date(karyawan.clockedInAt).toLocaleString()
              : '-'}
          </p>
          <p>
            <span className='font-medium text-gray-600'>Clocked Out:</span>{' '}
            {karyawan.clockedOutAt
              ? new Date(karyawan.clockedOutAt).toLocaleString()
              : 'Belum Clock Out'}
          </p>
          <p>
            <span className='font-medium text-gray-600'>Dibuat:</span>{' '}
            {new Date(karyawan.createdAt).toLocaleString()}
          </p>
          <p>
            <span className='font-medium text-gray-600'>
              Terakhir Diperbarui:
            </span>{' '}
            {new Date(karyawan.updatedAt).toLocaleString()}
          </p>
        </div>
